...
fun get(): Command = { arguments ->
    arguments.firstOrNull()?.let {key ->
        ...
        val response = when (key.firstOrNull()) {
            '+' -> constructRespType(parseRespType(Type.STRING, x))
            ':' -> constructRespType(parseRespType(Type.INT, x))
            '#' -> constructRespType(parseRespType(Type.BOOLEAN, x))
            '(' -> constructRespType(parseRespType(Type.BIGINTEGER, x))
            ',' -> constructRespType(parseRespType(Type.DOUBLE, x))
            '$' -> constructRespType(parseRespType(Type.BYTEARRAY, x))
            '*' -> constructRespType(parseRespType(Type.ARRAY, x))
            '%' -> constructRespType(parseRespType(Type.MAP, x))
            '~' -> constructRespType(parseRespType(Type.SET, x))
            '_' -> "_"
            null -> "_"
            else -> error("Invalid?")
        }
...
fun constructRespType(x: RespType): String {
    return cache[x]?.let {
        when(it) {
